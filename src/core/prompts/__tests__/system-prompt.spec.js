...
describe("SYSTEM_PROMPT", () => {
	...
	it("should include diff strategy tool description when diffEnabled is true", async () => {
		const prompt = await (0, system_1.SYSTEM_PROMPT)(
			...
			false, // supportsComputerUse
			undefined, // mcpHub
			new multi_search_replace_1.MultiSearchReplaceDiffStrategy(), // Use actual diff strategy from the codebase
			undefined, // browserViewportSize
			modes_1.defaultModeSlug, // mode
			undefined, // customModePrompts
			undefined, // customModes
	...
	it("should include vscode language in custom instructions", async () => {
		...
		const vscode = vi.mocked(await Promise.resolve().then(() => __importStar(require("vscode"))))
