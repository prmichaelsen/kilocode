...
const custom_system_prompt_1 = require("../custom-system-prompt")
...
describe("loadSystemPromptFile", () => {
	...
	const expectedFilePath = path_1.default.join(mockCwd, ".kilocode", `system-prompt-${mockMode}`)
	beforeEach(() => {
	...
	it("should return an empty string if the file does not exist (ENOENT)", async () => {
		...
		const result = await (0, custom_system_prompt_1.loadSystemPromptFile)(mockCwd, mockMode, mockVariables)
		expect(result).toBe("")
	...
	it("should re-throw unexpected errors from readFile", async () => {
		...
		await expect(
			(0, custom_system_prompt_1.loadSystemPromptFile)(mockCwd, mockMode, mockVariables),
