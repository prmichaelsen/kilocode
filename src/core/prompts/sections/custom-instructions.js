...
async function readTextFilesFromDirectory(dirPath) {
	try {
		...
		const { isBinaryFile } = await Promise.resolve().then(() => __importStar(require("isbinaryfile")))
...
async function addCustomInstructions(
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path_1.default.join(".kilocode", `rules-${mode}`))) {
	...
	if (
		(0, kilo_1.hasAnyToggles)(options.localRulesToggleState) ||
		(0, kilo_1.hasAnyToggles)(options.globalRulesToggleState)
	) {
		const genericRuleContent =
			(
				await (0, kilo_1.loadEnabledRules)(
					cwd,
