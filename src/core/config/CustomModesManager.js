...
class CustomModesManager {
	constructor(context, onUpdate) {
		...
		Object.defineProperty(this, "cachedModes", {
			enumerable: true,
			configurable: true,
			writable: true,
			value: null,
		})
		Object.defineProperty(this, "cachedAt", {
			enumerable: true,
			configurable: true,
			writable: true,
			value: 0,
		...
		try {
			...
			if (isGlobalMode) {
				...
				modeRulesDir = path.join(
					(0, roo_config_1.getProjectRooDirectoryForCwd)(workspacePath) /* kilocode_change */,
					`rules-${slug}`,
				)
			}
			try {
	...
	async exportModeWithRules(slug, customPrompts) {
		try {
			...
			const { modes: builtInModes } = await Promise.resolve().then(() =>
				__importStar(require("../../shared/modes")),
			)
			...
			const modeRulesDir = isGlobalMode
				...
				: path.join(
						(0, roo_config_1.getProjectRooDirectoryForCwd)(baseDir) /* kilocode_change */,
						`rules-${slug}`,
					)
			let rulesFiles = []
			try {
	...
	async importRulesFiles(importMode, rulesFiles, source) {
		...
		if (source === "global") {
			...
			const workspacePath = (0, path_1.getWorkspacePath)()
			baseDir = (0, roo_config_1.getProjectRooDirectoryForCwd)(workspacePath) // kilocode_change
			rulesFolderPath = path.join(baseDir, `rules-${importMode.slug}`)
		}
...
Object.defineProperty(CustomModesManager, "cacheTTL", {
	enumerable: true,
	configurable: true,
	writable: true,
	value: 10000,
 ...
 * - \u201C-\u201D: Smart double quotes
 */
Object.defineProperty(CustomModesManager, "PROBLEMATIC_CHARS_REGEX", {
	enumerable: true,
	configurable: true,
	writable: true,
	value:
