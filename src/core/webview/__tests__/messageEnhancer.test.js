...
describe("MessageEnhancer", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(telemetry_1.TelemetryService).hasInstance = vi.fn().mockReturnValue(true)
		// Mock the instance getter
		Object.defineProperty(telemetry_1.TelemetryService, "instance", {
	...
	describe("captureTelemetry", () => {
		it("should capture telemetry when TelemetryService is available", () => {
			...
			vi.mocked(telemetry_1.TelemetryService.instance).captureEvent = mockCaptureEvent
			messageEnhancer_1.MessageEnhancer.captureTelemetry(mockTaskId, true)
			expect(telemetry_1.TelemetryService.hasInstance).toHaveBeenCalled()
		...
		it("should handle missing TelemetryService gracefully", () => {
			vi.mocked(telemetry_1.TelemetryService).hasInstance = vi.fn().mockReturnValue(false)
			// Should not throw
			expect(() => messageEnhancer_1.MessageEnhancer.captureTelemetry("task-123", true)).not.toThrow()
		})
		it("should work without task ID", () => {
			const mockCaptureEvent = vi.fn()
			vi.mocked(telemetry_1.TelemetryService.instance).captureEvent = mockCaptureEvent
			messageEnhancer_1.MessageEnhancer.captureTelemetry(undefined, false)
			expect(mockCaptureEvent).toHaveBeenCalledWith(expect.any(String), {
		...
		it("should default includeTaskHistory to false when not provided", () => {
			const mockCaptureEvent = vi.fn()
			vi.mocked(telemetry_1.TelemetryService.instance).captureEvent = mockCaptureEvent
			messageEnhancer_1.MessageEnhancer.captureTelemetry("task-123")
			expect(mockCaptureEvent).toHaveBeenCalledWith(expect.any(String), {
				taskId: "task-123",
				includeTaskHistory: false,
			})
