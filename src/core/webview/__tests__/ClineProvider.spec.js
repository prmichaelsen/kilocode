...
const types_1 = require("@roo-code/types") // kilocode_change: openRouterDefaultModelId
...
vi.mocked(
	await Promise.resolve().then(() => __importStar(require("../../prompts/sections/custom-instructions"))),
...
describe("ClineProvider", () => {
	...
	test("postMessageToWebview sends message to webview", async () => {
		...
		const mockState = {
			...
			apiConfiguration: {
				...
				kilocodeModel: types_1.openRouterDefaultModelId,
				kilocodeToken: "kilocode-token",
				// kilocode_change end
			},
			kilocodeDefaultModel: types_1.openRouterDefaultModelId,
	...
	test("file content includes line numbers", async () => {
		const { extractTextFromFile } = await Promise.resolve().then(() =>
			__importStar(require("../../../integrations/misc/extract-text")),
		)
	...
	describe("getSystemPrompt", () => {
		...
		test("handles errors gracefully", async () => {
			...
			const { SYSTEM_PROMPT } = await Promise.resolve().then(() => __importStar(require("../../prompts/system")))
	...
	describe("createTaskWithHistoryItem mode validation", () => {
		test("validates and falls back to default mode when restored mode no longer exists", async () => {
			...
			const { getModeBySlug } = await Promise.resolve().then(() => __importStar(require("../../../shared/modes")))
	...
	describe("upsertApiConfiguration", () => {
		...
		test("handles buildApiHandler error in updateApiConfiguration", async () => {
			...
			const { buildApiHandler } = await Promise.resolve().then(() => __importStar(require("../../../api")))
	...
	describe("browser connection features", () => {
		...
		test("handles testBrowserConnection with auto-discovery", async () => {
			...
			const { discoverChromeHostUrl } = await Promise.resolve().then(() =>
				__importStar(require("../../../services/browser/browserDiscovery")),
			)
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		const fsUtils = await Promise.resolve().then(() => __importStar(require("../../../utils/fs")))
		vi.spyOn(fsUtils, "fileExistsAtPath").mockResolvedValue(false)
		// Mock openFile
		const openFileModule = await Promise.resolve().then(() =>
			__importStar(require("../../../integrations/misc/open-file")),
		)
		...
		expect(safeWriteJson_1.safeWriteJson).toHaveBeenCalledWith("/test/workspace/.roo/mcp.json", { mcpServers: {} })
...
describe.skip("getTelemetryProperties", () => {
	...
	beforeEach(() => {
		...
		mockCline = new Task_1.Task(defaultTaskOptions)
		mockCline.api = {
			getModel: vi.fn().mockReturnValue({
	...
	describe("cloud authentication telemetry", () => {
		...
		test("includes cloud authentication property when user is authenticated", async () => {
			...
			const { CloudService } = await Promise.resolve().then(() => __importStar(require("@roo-code/cloud")))
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with successful responses", async () => {
		...
		const { getModels } = await Promise.resolve().then(() =>
			__importStar(require("../../../api/providers/fetchers/modelCache")),
		)
