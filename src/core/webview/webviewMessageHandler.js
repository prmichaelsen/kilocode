...
const notifications_1 = require("../../integrations/notifications") // kilocode_change
const single_completion_handler_1 = require("../../utils/single-completion-handler") // kilocode_change
...
const kilorules_1 = require("./kilorules")
const mermaid_1 = require("../prompts/utilities/mermaid") // kilocode_change
const webviewMessageHandlerUtils_1 = require("../kilocode/webview/webviewMessageHandlerUtils") // kilocode_change
...
const seeNewChanges_1 = require("../checkpoints/kilocode/seeNewChanges") // kilocode_change
const getTaskHistory_1 = require("../../shared/kilocode/getTaskHistory") // kilocode_change
const webviewMessageHandler = async (provider, message, marketplaceManager) => {
	...
	switch (message.type) {
		case "webviewDidLaunch":
			...
			const { refreshWorkflowToggles } = await Promise.resolve().then(() =>
				__importStar(require("../context/instructions/workflows")),
			) // kilocode_change
			...
			provider.getStateToPostToWebview().then(async (/*kilocode_change*/ state) => {
				...
				await telemetry_1.TelemetryService.instance.updateIdentity(state.apiConfiguration.kilocodeToken ?? "") // kilocode_change
			})
		...
		case "shareCurrentTask":
			...
			try {
				...
				const result = await cloud_1.CloudService.instance.shareTask(shareTaskId, visibility)
		...
		case "requestHuggingFaceModels":
			...
			try {
				const { getHuggingFaceModelsWithMetadata } = await Promise.resolve().then(() =>
					__importStar(require("../../api/providers/fetchers/huggingface")),
				)
		...
		case "seeNewChanges":
			...
			if (task && message.payload && message.payload) {
				await (0, seeNewChanges_1.seeNewChanges)(task, message.payload.commitRange)
			}
			break
		...
		case "taskHistoryRequest": {
			await provider.postMessageToWebview({
				...
				payload: (0, getTaskHistory_1.getTaskHistory)(provider.getTaskHistory(), provider.cwd, message.payload),
			})
			break
		}
		// kilocode_change end
		...
		case "openProjectMcpSettings": {
			...
			try {
				...
				;(0, open_file_1.openFile)(mcpPath)
			} catch (error) {
		...
		case "showSystemNotification":
			...
			if (message.notificationOptions) {
				;(0, notifications_1.showSystemNotification)(message.notificationOptions)
			}
			break
		...
		case "remoteControlEnabled":
			try {
				await cloud_1.CloudService.instance.updateUserSettings({
		...
		case "morphApiKey":
			...
			break
		case "fastApplyModel": {
			const nextModel = types_1.fastApplyModelSchema.safeParse(message.text).data ?? "auto"
		...
		case "terminalOutputLineLimit":
			...
			if (typeof lineLimit === "number" && lineLimit > 0) {
				...
				vscode.window.showErrorMessage(
					(0, i18n_1.t)("common:errors.invalid_line_limit") ||
						"Terminal output line limit must be a positive number",
				)
			}
			break
		case "terminalOutputCharacterLimit":
			...
			if (typeof charLimit === "number" && charLimit > 0) {
				...
				vscode.window.showErrorMessage(
					(0, i18n_1.t)("common:errors.invalid_character_limit") ||
		...
		case "terminalShellIntegrationTimeout":
			...
			if (message.value !== undefined) {
				Terminal_1.Terminal.setShellIntegrationTimeout(message.value)
			}
			break
		case "terminalShellIntegrationDisabled":
			...
			if (message.bool !== undefined) {
				Terminal_1.Terminal.setShellIntegrationDisabled(message.bool)
			}
			break
		case "terminalCommandDelay":
			...
			await provider.postStateToWebview()
			if (message.value !== undefined) {
				Terminal_1.Terminal.setCommandDelay(message.value)
			}
			break
		case "terminalPowershellCounter":
			...
			await provider.postStateToWebview()
			if (message.bool !== undefined) {
				Terminal_1.Terminal.setPowershellCounter(message.bool)
			}
			break
		case "terminalZshClearEolMark":
			...
			await provider.postStateToWebview()
			if (message.bool !== undefined) {
				Terminal_1.Terminal.setTerminalZshClearEolMark(message.bool)
			}
			break
		case "terminalZshOhMy":
			...
			await provider.postStateToWebview()
			if (message.bool !== undefined) {
				Terminal_1.Terminal.setTerminalZshOhMy(message.bool)
			}
			break
		case "terminalZshP10k":
			...
			await provider.postStateToWebview()
			if (message.bool !== undefined) {
				Terminal_1.Terminal.setTerminalZshP10k(message.bool)
			}
			break
		case "terminalZdotdir":
			...
			await provider.postStateToWebview()
			if (message.bool !== undefined) {
				Terminal_1.Terminal.setTerminalZdotdir(message.bool)
			}
			break
		case "terminalCompressProgressBar":
			...
			await provider.postStateToWebview()
			if (message.bool !== undefined) {
				Terminal_1.Terminal.setCompressProgressBar(message.bool)
			}
			break
		case "mode":
		...
		case "updateSupportPrompt":
			try {
				...
				vscode.window.showErrorMessage((0, i18n_1.t)("common:errors.update_support_prompt"))
			}
			break
		case "updatePrompt":
		...
		case "browserToolEnabled":
			...
			break
		case "language":
			;(0, i18n_1.changeLanguage)(message.text ?? "en")
		...
		case "ghostServiceSettings":
			...
			const ghostServiceSettings = types_1.ghostServiceSettingsSchema.parse(message.values)
			...
			if (message.text) {
				try {
					...
					if (result.success && result.enhancedText) {
						...
						messageEnhancer_1.MessageEnhancer.captureTelemetry(
							currentCline?.taskId,
							includeTaskHistoryInEnhance,
						)
					...
					telemetry_1.TelemetryService.instance.captureException(error, { context: "enhance_prompt" }) // kilocode_change
		...
		case "showFeedbackOptions": {
			const githubIssuesText = (0, i18n_1.t)("common:feedback.githubIssues")
			const discordText = (0, i18n_1.t)("common:feedback.discord")
			const customerSupport = (0, i18n_1.t)("common:feedback.customerSupport")
			const answer = await vscode.window.showInformationMessage(
				(0, i18n_1.t)("common:feedback.description"),
				{ modal: true },
				githubIssuesText,
				discordText,
				customerSupport,
			...
			if (message.text && message.apiConfiguration) {
				...
				try {
					...
					if (currentConfig.kilocodeOrganizationId !== message.apiConfiguration.kilocodeOrganizationId) {
						await (0, modelCache_1.flushModels)("kilocode-openrouter")
			...
			if (message.modeConfig) {
				...
				if (telemetry_1.TelemetryService.hasInstance()) {
					if (isNewMode) {
						...
						telemetry_1.TelemetryService.instance.captureCustomModeCreated(
		...
		case "fetchProfileDataRequest":
			try {
				...
				const response = await axios_1.default.get(
					`${(0, token_1.getKiloBaseUriFromToken)(kilocodeToken)}/api/profile`,
					{
						headers,
					},
				)
		...
		case "fetchBalanceDataRequest": // New handler
			try {
				...
				const response = await axios_1.default.get(
					`${(0, token_1.getKiloBaseUriFromToken)(kilocodeToken)}/api/profile/balance`,
					{
		...
		case "shopBuyCredits": // New handler
			try {
				...
				const baseUrl = (0, token_1.getKiloBaseUriFromToken)(kilocodeToken)
		...
		case "toggleWorkflow": {
			if (message.workflowPath && typeof message.enabled === "boolean" && typeof message.isGlobal === "boolean") {
				await (0, kilorules_1.toggleWorkflow)(
		...
		case "toggleRule": {
			if (message.rulePath && typeof message.enabled === "boolean" && typeof message.isGlobal === "boolean") {
				await (0, kilorules_1.toggleRule)(
					message.rulePath,
					message.enabled,
					message.isGlobal,
					provider.contextProxy,
		...
		case "createRuleFile": {
			if (
				...
				try {
					await (0, kilorules_1.createRuleFile)(message.filename, message.isGlobal, message.ruleType)
				} catch (error) {
					console.error("Error creating rule file:", error)
					vscode.window.showErrorMessage((0, i18n_1.t)("kilocode:rules.errors.failedToCreateRuleFile"))
				}
				await provider.postRulesDataToWebview()
			}
			break
		...
		case "deleteRuleFile": {
			if (message.rulePath) {
				try {
					await (0, kilorules_1.deleteRuleFile)(message.rulePath)
				} catch (error) {
					console.error("Error deleting rule file:", error)
					vscode.window.showErrorMessage((0, i18n_1.t)("kilocode:rules.errors.failedToDeleteRuleFile"))
				}
				await provider.postRulesDataToWebview()
			}
			break
		...
		case "rooCloudManualUrl": {
			try {
				...
				const errorMessage =
					error instanceof Error ? error.message : (0, i18n_1.t)("common:errors.manual_url_auth_failed")
				// Show error message through VS Code UI
				vscode.window.showErrorMessage(
					`${(0, i18n_1.t)("common:errors.manual_url_auth_error")}: ${errorMessage}`,
				)
			}
			break
		}
		...
		case "clearUsageData": {
			try {
				const usageTracker = usage_tracker_1.UsageTracker.getInstance()
		...
		case "fixMermaidSyntax":
			if (message.text && message.requestId) {
				try {
					const { apiConfiguration } = await provider.getState()
					const prompt = (0, mermaid_1.mermaidFixPrompt)(
						message.values?.error || "Unknown syntax error",
						message.text,
					)
					const fixedCode = await (0, single_completion_handler_1.singleCompletionHandler)(
						apiConfiguration,
						prompt,
					)
					provider.postMessageToWebview({
		...
		// kilocode_change start
		case "editMessage": {
			await (0, webviewMessageHandlerUtils_1.editMessageHandler)(provider, message)
			break
		}
		case "fetchKilocodeNotifications": {
			await (0, webviewMessageHandlerUtils_1.fetchKilocodeNotificationsHandler)(provider)
			break
		}
		...
		case "requestCommands": {
			try {
				const { getCommands } = await Promise.resolve().then(() =>
					__importStar(require("../../services/command/commands")),
				)
		...
		case "getKeybindings": {
			try {
				const { getKeybindingsForCommands } = await Promise.resolve().then(() =>
					__importStar(require("../../utils/keybindings")),
				)
		...
		case "openCommandFile": {
			try {
				if (message.text) {
					const { getCommand } = await Promise.resolve().then(() =>
						__importStar(require("../../services/command/commands")),
					)
