...
const native_ollama_1 = require("../../api/providers/native-ollama")
// kilocode_change end
async function getEnvironmentDetails(cline, includeFileDetails = false) {
	...
	if (busyTerminals.length > 0) {
		...
		for (const busyTerminal of busyTerminals) {
			...
			if (newOutput) {
				newOutput = Terminal_1.Terminal.compressTerminalOutput(
					newOutput,
	...
	if (terminalsWithOutput.length > 0) {
		...
		for (const inactiveTerminal of terminalsWithOutput) {
			...
			for (const process of completedProcesses) {
				...
				if (output) {
					output = Terminal_1.Terminal.compressTerminalOutput(
						output,
						terminalOutputLineLimit,
						terminalOutputCharacterLimit,
					)
	...
	const { contextTokens, totalCost } = (0, getApiMetrics_1.getApiMetrics)(cline.clineMessages)
	// kilocode_change start
	// Be sure to fetch the model information before we need it.
	if (
		cline.api instanceof openrouter_1.OpenRouterHandler ||
		cline.api instanceof native_ollama_1.NativeOllamaHandler
	) {
		try {
			await cline.api.fetchModel()
		} catch (e) {
			telemetry_1.TelemetryService.instance.captureException(e, { context: "getEnvironmentDetails" })
			await cline.say(
				"error",
				(0, i18n_1.t)("kilocode:task.notLoggedInError", { error: e instanceof Error ? e.message : String(e) }),
			)
	...
	const modeDetails = await (0, modes_1.getFullModeDetails)(
