...
exports.parseKiloSlashCommands = parseKiloSlashCommands
 ...
 */
async function parseKiloSlashCommands(text, localWorkflowToggles, globalWorkflowToggles) {
	const commandReplacements = {
		newtask: commands_1.newTaskToolResponse,
		newrule: commands_1.newRuleToolResponse,
		reportbug: commands_1.reportBugToolResponse,
		smol: commands_1.condenseToolResponse,
	}
			...
			if (matchingWorkflow) {
				try {
					...
					const workflowContent = (
						await promises_1.default.readFile(matchingWorkflow.fullPath, "utf8")
					).trim()
					// find position of slash command within the full match
					const fullMatchStartIndex = match.index
					const fullMatch = match[0]
