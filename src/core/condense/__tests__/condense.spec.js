...
describe("Condense", () => {
	...
	describe("summarizeConversation", () => {
		it("should preserve the first message when summarizing", async () => {
			...
			expect(result.messages.length).toBe(1 + 1 + index_1.N_MESSAGES_TO_KEEP)
			// Verify the last N messages are preserved
			const lastMessages = result.messages.slice(-index_1.N_MESSAGES_TO_KEEP)
			expect(lastMessages).toEqual(messages.slice(-index_1.N_MESSAGES_TO_KEEP))
		})
