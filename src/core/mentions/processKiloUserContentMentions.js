...
exports.processKiloUserContentMentions = processKiloUserContentMentions
const index_1 = require("./index")
const globalFileNames_1 = require("../../shared/globalFileNames")
const kilo_rules_1 = require("../context/instructions/kilo-rules")
const kilo_1 = require("../slash-commands/kilo")
...
async function processKiloUserContentMentions({
			...
			userContent.map(async (block) => {
				...
				if (block.type === "text") {
					if (shouldProcessMentions(block.text)) {
						...
						const parsedText = await (0, index_1.parseMentions)(
							block.text,
							cwd,
							urlContentFetcher,
							fileContextTracker,
					...
					if (typeof block.content === "string") {
						if (shouldProcessMentions(block.content)) {
							return {
								...block,
								content: await (0, index_1.parseMentions)(
									block.content,
									cwd,
									urlContentFetcher,
									fileContextTracker,
						...
						const parsedContent = await Promise.all(
							block.content.map(async (contentBlock) => {
								if (contentBlock.type === "text" && shouldProcessMentions(contentBlock.text)) {
									return {
										...contentBlock,
										text: await (0, index_1.parseMentions)(
											contentBlock.text,
											cwd,
											urlContentFetcher,
											fileContextTracker,
	...
	if (needsRulesFileCheck) {
		kilorulesError = await (0, kilo_rules_1.ensureLocalKilorulesDirExists)(
			cwd,
