...
async function askFollowupQuestionTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
	...
	try {
		if (block.partial) {
			...
			if (follow_up) {
				let parsedSuggest
				try {
					parsedSuggest = (0, xml_1.parseXml)(follow_up, ["suggest"])
				} catch (error) {
					cline.consecutiveMistakeCount++
					cline.recordToolError("ask_followup_question")
					await cline.say("error", `Failed to parse operations: ${error.message}`)
					pushToolResult(responses_1.formatResponse.toolError("Invalid operations xml format"))
					return
				}
