...
exports.summarizeSuccessfulMcpOutputWhenTooLong = summarizeSuccessfulMcpOutputWhenTooLong
exports.blockFileReadWhenTooLarge = blockFileReadWhenTooLarge
...
async function summarizeSuccessfulMcpOutputWhenTooLong(task, outputText) {
...
async function blockFileReadWhenTooLarge(task, relPath, content) {
	if (await allowVeryLargeReads(task)) {
		return undefined
	}
	const tokenLimit = getTokenLimit(task)
