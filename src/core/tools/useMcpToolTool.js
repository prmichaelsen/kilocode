...
const kilocode_1 = require("./kilocode") // kilocode_change
...
async function validateParams(cline, params, pushToolResult) {
	...
	if (params.arguments) {
		try {
			...
			cline.consecutiveMistakeCount++
			cline.recordToolError("use_mcp_tool")
			await cline.say("error", (0, i18n_1.t)("mcp:errors.invalidJsonArgument", { toolName: params.tool_name }))
			pushToolResult(
...
async function validateToolExists(cline, serverName, toolName, pushToolResult) {
	try {
		...
		if (!server) {
			...
			cline.consecutiveMistakeCount++
			cline.recordToolError("use_mcp_tool")
			await cline.say("error", (0, i18n_1.t)("mcp:errors.serverNotFound", { serverName, availableServers }))
...
async function processToolContent(task, toolResult) {
	...
	return toolResult.isError
		? outputText
		: await (0, kilocode_1.summarizeSuccessfulMcpOutputWhenTooLong)(task, outputText)
}
...
async function useMcpToolTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
	try {
		const params = {
