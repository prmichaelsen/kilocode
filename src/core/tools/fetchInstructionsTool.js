...
async function fetchInstructionsTool(cline, block, askApproval, handleError, pushToolResult) {
	...
	try {
		if (block.partial) {
			...
			const content = await (0, instructions_1.fetchInstructions)(task, { mcpHub, diffStrategy, context })
			if (!content) {
				pushToolResult(responses_1.formatResponse.toolError(`Invalid instructions request: ${task}`))
				return
			}
