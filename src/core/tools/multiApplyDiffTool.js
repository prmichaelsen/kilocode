...
async function applyDiffTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
	...
	if (provider) {
		...
		if (!isMultiFileApplyDiffEnabled) {
			return (0, applyDiffTool_1.applyDiffToolLegacy)(
				cline,
				block,
				askApproval,
				handleError,
...
Original error: ${errorMessage}`
			...
			telemetry_1.TelemetryService.instance.captureDiffApplicationError(
				cline.taskId,
				cline.consecutiveMistakeCount,
			)
			...
			try {
				...
				const processedDiffItems = !cline.api.getModel().id.includes("claude")
					? diffItems.map((item) => ({
							...item,
							content: item.content
								? (0, text_normalization_1.unescapeHtmlEntities)(item.content)
								: item.content,
						}))
					: diffItems
...
${errorDetails ? `\nTechnical details:\n${errorDetails}\n` : ""}
				...
				if (operationsToApprove.length === 1) {
					...
					if (!isPreventFocusDisruptionEnabled) {
						...
						cline.diffViewProvider.originalContent = await promises_1.default.readFile(
							absolutePath,
							"utf-8",
						)
					}
