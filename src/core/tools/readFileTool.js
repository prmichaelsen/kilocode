...
exports.getReadFileToolDescription = getReadFileToolDescription
...
function getReadFileToolDescription(blockName, blockParams) {
...
async function readFileTool(cline, block, askApproval, handleError, pushToolResult, _removeClosingTag) {
	const argsXmlTag = block.params.args
	const legacyPath = block.params.path
	const legacyStartLineStr = block.params.start_line
	const legacyEndLineStr = block.params.end_line
	...
	const updateFileResult = (path, updates) => {
	...
	try {
			...
			const batchFiles = filesToApprove.map((fileResult) => {
				...
				if (fileResult.lineRanges && fileResult.lineRanges.length > 0) {
					const ranges = fileResult.lineRanges.map((range) =>
						(0, i18n_1.t)("tools:readFile.linesRange", { start: range.start, end: range.end }),
					)
				...
				const readablePath = (0, path_2.getReadablePath)(cline.cwd, relPath)
		...
		for (const fileResult of fileResults) {
			...
			try {
				const [totalLines, isBinary] = await Promise.all([
					(0, line_counter_1.countFileLines)(fullPath),
					(0, isbinaryfile_1.isBinaryFile)(fullPath),
				])
				...
				if (fileResult.lineRanges && fileResult.lineRanges.length > 0) {
					const rangeResults = []
					for (const range of fileResult.lineRanges) {
						const content = (0, extract_text_1.addLineNumbers)(
							await (0, read_lines_1.readLines)(fullPath, range.end - 1, range.start - 1),
							range.start,
						)
				...
				if (maxReadFileLine === 0) {
					try {
						const defResult = await (0, tree_sitter_1.parseSourceCodeDefinitionsForFile)(
							fullPath,
				...
				if (maxReadFileLine > 0 && totalLines > maxReadFileLine) {
					const content = (0, extract_text_1.addLineNumbers)(
						await (0, read_lines_1.readLines)(fullPath, maxReadFileLine - 1, 0),
					)
				...
				const blockResult = await (0, kilocode_1.blockFileReadWhenTooLarge)(cline, relPath, content)
				if (blockResult) {
		...
		const fileImageUrls = fileResults.filter((result) => result.imageDataUrl).map((result) => result.imageDataUrl)
