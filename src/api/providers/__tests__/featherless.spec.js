...
const featherless_1 = require("../featherless")
...
describe("FeatherlessHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new featherless_1.FeatherlessHandler({ featherlessApiKey: "test-key" })
	})
	afterEach(() => {
		vi.restoreAllMocks()
	})
	it("should use the correct Featherless base URL", () => {
		new featherless_1.FeatherlessHandler({ featherlessApiKey: "test-featherless-api-key" })
	...
	it("should use the provided API key", () => {
		...
		new featherless_1.FeatherlessHandler({ featherlessApiKey })
		expect(openai_1.default).toHaveBeenCalledWith(expect.objectContaining({ apiKey: featherlessApiKey }))
	})
	...
	it("should return default model when no model is specified", () => {
		...
		expect(model.id).toBe(types_1.featherlessDefaultModelId)
		expect(model.info).toEqual(
			expect.objectContaining(types_1.featherlessModels[types_1.featherlessDefaultModelId]),
		)
	})
	it("should return specified model when valid model is provided", () => {
		const testModelId = "deepseek-ai/DeepSeek-R1-0528"
		const handlerWithModel = new featherless_1.FeatherlessHandler({
			apiModelId: testModelId,
			featherlessApiKey: "test-featherless-api-key",
		})
		const model = handlerWithModel.getModel()
		expect(model.id).toBe(testModelId)
		expect(model.info).toEqual(expect.objectContaining(types_1.featherlessModels[testModelId]))
	})
