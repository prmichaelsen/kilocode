...
describe("CerebrasHandler", () => {
	...
	beforeEach(() => {
		vi.clearAllMocks()
		handler = new cerebras_1.CerebrasHandler(mockOptions)
	})
	describe("constructor", () => {
		it("should throw error when API key is missing", () => {
			expect(() => new cerebras_1.CerebrasHandler({ cerebrasApiKey: "" })).toThrow("Cerebras API key is required")
		})
		it("should initialize with valid API key", () => {
			expect(() => new cerebras_1.CerebrasHandler(mockOptions)).not.toThrow()
		})
	})
	describe("getModel", () => {
		it("should return correct model info", () => {
			...
			expect(info).toEqual(types_1.cerebrasModels["llama-3.3-70b"])
		})
		it("should fallback to default model when apiModelId is not provided", () => {
			const handlerWithoutModel = new cerebras_1.CerebrasHandler({ cerebrasApiKey: "test" })
	...
	describe("createMessage", () => {
		...
		it("should handle temperature clamping", async () => {
			const handlerWithTemp = new cerebras_1.CerebrasHandler({
				...mockOptions,
