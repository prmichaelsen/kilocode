...
describe("OllamaHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new ollama_1.OllamaHandler(mockOptions)
		mockCreate.mockClear()
	})
	describe("constructor", () => {
		it("should initialize with provided options", () => {
			expect(handler).toBeInstanceOf(ollama_1.OllamaHandler)
			expect(handler.getModel().id).toBe(mockOptions.ollamaModelId)
		})
		it("should use default base URL if not provided", () => {
			const handlerWithoutUrl = new ollama_1.OllamaHandler({
				apiModelId: "llama2",
				ollamaModelId: "llama2",
			})
			expect(handlerWithoutUrl).toBeInstanceOf(ollama_1.OllamaHandler)
		})
		it("should use API key when provided", () => {
			const handlerWithApiKey = new ollama_1.OllamaHandler({
				apiModelId: "llama2",
				ollamaModelId: "llama2",
				ollamaBaseUrl: "https://ollama.com",
				ollamaApiKey: "test-api-key",
			})
			expect(handlerWithApiKey).toBeInstanceOf(ollama_1.OllamaHandler)
