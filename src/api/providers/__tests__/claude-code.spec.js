...
const { runClaudeCode } = await Promise.resolve().then(() =>
	__importStar(require("../../../integrations/claude-code/run")),
)
const { filterMessagesForClaudeCode } = await Promise.resolve().then(() =>
	__importStar(require("../../../integrations/claude-code/message-filter")),
)
...
describe("ClaudeCodeHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new claude_code_1.ClaudeCodeHandler(options)
	})
	...
	test("should use default model when invalid model provided", () => {
		...
		const handlerWithInvalidModel = new claude_code_1.ClaudeCodeHandler(options)
	...
	test("should override maxTokens when claudeCodeMaxOutputTokens is provided", () => {
		...
		const handlerWithMaxTokens = new claude_code_1.ClaudeCodeHandler(options)
