...
describe("ChutesHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new chutes_1.ChutesHandler({ chutesApiKey: "test-key" })
	})
	afterEach(() => {
		vi.restoreAllMocks()
	})
	it("should use the correct Chutes base URL", () => {
		new chutes_1.ChutesHandler({ chutesApiKey: "test-chutes-api-key" })
		expect(openai_1.default).toHaveBeenCalledWith(expect.objectContaining({ baseURL: "https://llm.chutes.ai/v1" }))
	})
	it("should use the provided API key", () => {
		const chutesApiKey = "test-chutes-api-key"
		new chutes_1.ChutesHandler({ chutesApiKey })
		expect(openai_1.default).toHaveBeenCalledWith(expect.objectContaining({ apiKey: chutesApiKey }))
	})
	...
	it("should return default model when no model is specified", () => {
		...
		expect(model.id).toBe(types_1.chutesDefaultModelId)
		expect(model.info).toEqual(expect.objectContaining(types_1.chutesModels[types_1.chutesDefaultModelId]))
	})
	it("should return specified model when valid model is provided", () => {
		const testModelId = "deepseek-ai/DeepSeek-R1"
		const handlerWithModel = new chutes_1.ChutesHandler({
			apiModelId: testModelId,
			chutesApiKey: "test-chutes-api-key",
		})
		const model = handlerWithModel.getModel()
		expect(model.id).toBe(testModelId)
		expect(model.info).toEqual(expect.objectContaining(types_1.chutesModels[testModelId]))
	})
	...
	it("should return zai-org/GLM-4.5-turbo model with correct configuration", () => {
		const testModelId = "zai-org/GLM-4.5-turbo"
		const handlerWithModel = new chutes_1.ChutesHandler({
			apiModelId: testModelId,
			chutesApiKey: "test-chutes-api-key",
		})
	...
	it("createMessage should pass correct parameters to Chutes client for non-DeepSeek models", async () => {
		...
		const modelInfo = types_1.chutesModels[modelId]
		const handlerWithModel = new chutes_1.ChutesHandler({
			apiModelId: modelId,
			chutesApiKey: "test-chutes-api-key",
		})
	...
	it("should apply DeepSeek default temperature for R1 models", () => {
		...
		const model = handlerWithModel.getModel()
		expect(model.info.temperature).toBe(types_1.DEEP_SEEK_DEFAULT_TEMPERATURE)
	})
