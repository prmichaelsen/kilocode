...
const constants_1 = require("../constants") // kilocode_change
/**
 * OpenRouterBaseModel
 */
...
const modelRouterBaseModelSchema = zod_1.z.object({
	...
	preferredIndex: zod_1.z.number().nullish(), // kilocode_change
...
exports.openRouterModelEndpointSchema = modelRouterBaseModelSchema.extend({
	model_name: zod_1.z.string(), // kilocode_change
...
async function getOpenRouterModels(options) {
	...
	try {
		...
		const response = await fetch(`${baseURL}/models`, {
			headers: { ...constants_1.DEFAULT_HEADERS, ...(options?.headers ?? {}) },
		})
		...
		for (const model of data) {
			...
			models[id] = (0, exports.parseOpenRouterModel)({
				id,
				model,
...
const parseOpenRouterModel = ({
	...
	if (types_1.OPEN_ROUTER_COMPUTER_USE_MODELS.has(id)) {
