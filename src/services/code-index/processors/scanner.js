...
class DirectoryScanner {
	constructor(embedder, qdrantClient, codeParser, cacheManager, ignoreInstance, batchSegmentThreshold) {
		...
		Object.defineProperty(this, "qdrantClient", {
			enumerable: true,
			configurable: true,
			writable: true,
			value: qdrantClient,
		})
		Object.defineProperty(this, "codeParser", {
			enumerable: true,
			configurable: true,
			writable: true,
			value: codeParser,
			...
			parseLimiter(async () => {
				...
				try {
					...
					telemetry_1.TelemetryService.instance.captureEvent(types_1.TelemetryEventName.CODE_INDEX_ERROR, {
						error: (0, validation_helpers_1.sanitizeErrorMessage)(
							error instanceof Error ? error.message : String(error),
						),
						stack:
							error instanceof Error
								? (0, validation_helpers_1.sanitizeErrorMessage)(error.stack || "")
								: undefined,
	...
	async processBatch(batchBlocks, batchTexts, batchFileInfos, scanWorkspace, onError, onBlocksIndexed) {
		...
		while (attempts < constants_1.MAX_BATCH_RETRIES && !success) {
			...
			console.debug(
				`[DirectoryScanner] Processing batch attempt ${attempts}/${constants_1.MAX_BATCH_RETRIES} for ${batchBlocks.length} blocks`,
			)
			// kilocode_change end
			try {
				// --- Deletion Step ---
