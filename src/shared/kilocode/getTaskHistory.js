...
const highlight_1 = require("../../../webview-ui/src/utils/highlight") // weird hack, but apparently it works
const PAGE_SIZE = 10
function getTaskHistory(taskHistory, cwd, request) {
	...
	if (request.search) {
		const searchResults = new fzf_1.Fzf(tasks, {
		...
		tasks = searchResults.map((result) => {
			...
			return {
				...result.item,
				highlight: (0, highlight_1.highlightFzfMatch)(
					result.item.task,
