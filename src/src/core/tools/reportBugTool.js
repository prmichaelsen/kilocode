...
export async function reportBugTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        if (block.partial) {
            ...
            const kilocodeVersion = vscode.extensions.getExtension("kilocode.kilo-code")?.packageJSON.version || "Unknown";
            ...
            if (text || images?.length) {
                ...
                pushToolResult(formatResponse.toolResult(`The user provided feedback on the Github issue generated:\n<feedback>\n${text}\n</feedback>`, images));
            }
            else {
