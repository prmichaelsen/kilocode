...
export async function codebaseSearchTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag) {
    ...
    try {
        ...
        if (status.systemStatus !== "Indexed") {
            ...
            const normalizedMessage = status.message && status.message.trim() !== "" ? status.message.trim() : defaultStatusMessage;
            const unit = status.currentItemUnit && status.currentItemUnit.trim() !== "" ? status.currentItemUnit : "items";
            ...
            pushToolResult(formatResponse.toolError(`${friendlyMessage} Semantic search is unavailable until indexing completes. Please try again later.`));
            return;
        }
        // kilocode_change end
