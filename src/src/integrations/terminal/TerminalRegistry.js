        ...
        try {
            const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(async (e) => {
                ...
                if (terminal) {
                    ...
                    console.error("[onDidStartTerminalShellExecution] Shell execution started, but not from a Kilo Code-registered terminal:", e);
                }
            });
            if (startDisposable) {
            ...
            const endDisposable = vscode.window.onDidEndTerminalShellExecution?.(async (e) => {
                ...
                if (!terminal) {
                    console.error("[onDidEndTerminalShellExecution] Shell execution ended, but not from a Kilo Code-registered terminal:", e);
                    return;
                }
                if (!terminal.running) {
