...
export class NativeOllamaHandler extends BaseProvider {
    ...
    async *createMessage(systemPrompt, messages, metadata) {
        ...
        if (estimatedTokenCount > maxTokens) {
            throw new Error(`Prompt is too long (estimated tokens: ${estimatedTokenCount}, max tokens: ${maxTokens}). Increase the Context Window Size in Settings.`);
        }
        // kilocode_change end
    ...
    async fetchModel() {
        // kilocode_change start
        this.models = await getOllamaModels(this.options.ollamaBaseUrl, this.options.ollamaApiKey, this.options.ollamaNumCtx);
        return this.models;
        // kilocode_change end
    }
    getModel() {
