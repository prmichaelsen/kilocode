...
export class DirectoryScanner {
    ...
    async processBatch(batchBlocks, batchTexts, batchFileInfos, scanWorkspace, onError, onBlocksIndexed) {
        ...
        if (this._cancelled || batchBlocks.length === 0)
            return;
        if (batchBlocks.length === 0) {
            console.debug("[DirectoryScanner] Skipping empty batch processing");
            return;
        }
        console.debug(`[DirectoryScanner] Starting to process batch of ${batchBlocks.length} blocks in workspace ${scanWorkspace}`);
        // kilocode_change end
        let attempts = 0;
        let success = false;
        let lastError = null;
        while (attempts < MAX_BATCH_RETRIES && !success) {
            ...
            if (this._cancelled)
                return;
            console.debug(`[DirectoryScanner] Processing batch attempt ${attempts}/${MAX_BATCH_RETRIES} for ${batchBlocks.length} blocks`);
            // kilocode_change end
            try {
                // --- Deletion Step ---
                ...
                // kilocode_change start
                console.debug(`[DirectoryScanner] Identified ${uniqueFilePaths.length} modified files to delete points for`);
                // kilocode_change end
                if (uniqueFilePaths.length > 0) {
                    try {
                        await this.qdrantClient.deletePointsByMultipleFilePaths(uniqueFilePaths);
                        // kilocode_change start
                        console.debug(`[DirectoryScanner] Successfully deleted points for ${uniqueFilePaths.length} files`);
                        // kilocode_change end
                    }
                    catch (deleteError) {
                ...
                success = true;
                // kilocode_change start
                console.debug(`[DirectoryScanner] Successfully processed batch of ${batchBlocks.length} blocks after ${attempts} attempt(s)`);
                // kilocode_change end
            }
            catch (error) {
                lastError = error;
