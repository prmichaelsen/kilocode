...
import type { IVSCodeAdapter, IProviderAdapter, IFileSystemWatcher, IDisposable } from "./interfaces"
...
const createServerTypeSchema = (getDefaultCwd: () => string) => {
	return z.union([
		...
		BaseConfigSchema.extend({
			...
			.refine((data: any) => data.type === undefined || data.type === "stdio", { message: typeErrorMessage }),
		...
		BaseConfigSchema.extend({
			...
			.refine((data: any) => data.type === undefined || data.type === "sse", { message: typeErrorMessage }),
		...
		BaseConfigSchema.extend({
			...
			.refine((data: any) => data.type === undefined || data.type === "streamable-http", {
...
const McpSettingsSchema = z.object({
	mcpServers: z.record(z.any()), // Will be validated by ServerConfigSchema
})

export class McpHub {
	private providerRef: WeakRef<IProviderAdapter>
	private vscodeAdapter: IVSCodeAdapter
	private disposables: IDisposable[] = []
	private settingsWatcher?: IFileSystemWatcher
	private fileWatchers: Map<string, FSWatcher[]> = new Map()
	private projectMcpWatcher?: IFileSystemWatcher
	...
	private ServerConfigSchema: z.ZodType<any>
	private t: (key: string, params?: any) => string

	constructor(provider: IProviderAdapter, vscodeAdapter: IVSCodeAdapter, t: (key: string, params?: any) => string) {
		this.providerRef = new WeakRef(provider)
		this.vscodeAdapter = vscodeAdapter
		this.t = t
		this.kiloNotificationService = new NotificationService(vscodeAdapter.window)
		
		// Create schema with proper default cwd function
		this.ServerConfigSchema = createServerTypeSchema(() => 
			this.vscodeAdapter.workspace.workspaceFolders?.at(0)?.uri.fsPath ?? process.cwd()
		)
	 ...
	 */
	private validateServerConfig(config: any, serverName?: string): z.infer<typeof this.ServerConfigSchema> {
	...
	public setupWorkspaceFoldersWatcher(): void {
		...
		this.disposables.push(
			this.vscodeAdapter.workspace.onDidChangeWorkspaceFolders(async () => {
	...
	private async handleConfigFileChange(filePath: string, source: "global" | "project"): Promise<void> {
		try {
			...
			try {
				...
				const errorMessage = this.t("mcp:errors.invalid_settings_syntax")
				console.error(errorMessage, parseError)
				this.vscodeAdapter.window.showErrorMessage(errorMessage)
				return
			}
			...
			if (!result.success) {
				const errorMessages = result.error.errors
					.map((err: any) => `${err.path.join(".")}: ${err.message}`)
					.join("\n")
				this.vscodeAdapter.window.showErrorMessage(this.t("mcp:errors.invalid_settings_validation", { errorMessages }))
				return
			}
			...
			if (error.code === "ENOENT" && source === "project") {
				...
				this.vscodeAdapter.window.showInformationMessage(this.t("mcp:info.project_config_deleted"))
			} else {
				this.showErrorMessage(this.t("mcp:errors.failed_update_project"), error)
			}
		}
	}
	...
	private async watchProjectMcpFile(): Promise<void> {
		...
		if (!this.vscodeAdapter.workspace.workspaceFolders?.length) {
			return
		}

		const workspaceFolder = this.providerRef.deref()?.cwd ?? this.vscodeAdapter.workspace.workspaceFolders[0]?.uri.fsPath ?? process.cwd()
		const projectMcpPattern = this.vscodeAdapter.createRelativePattern(workspaceFolder, ".kilocode/mcp.json")

		// Create a file system watcher for the project MCP file pattern
		this.projectMcpWatcher = this.vscodeAdapter.workspace.createFileSystemWatcher(projectMcpPattern)

		// Watch for file changes
		const changeDisposable = this.projectMcpWatcher.onDidChange((uri: { fsPath: string }) => {
			this.debounceConfigChange(uri.fsPath, "project")
		})

		// Watch for file creation
		const createDisposable = this.projectMcpWatcher.onDidCreate((uri: { fsPath: string }) => {
			this.debounceConfigChange(uri.fsPath, "project")
		})
		...
		this.disposables.push(
			this.vscodeAdapter.createDisposable([changeDisposable, createDisposable, deleteDisposable, this.projectMcpWatcher]),
		)
	}
  ...
  "mcpServers": {
	...
	private async watchMcpSettingsFile(): Promise<void> {
		...
		const settingsPattern = this.vscodeAdapter.createRelativePattern(path.dirname(settingsPath), path.basename(settingsPath))

		// Create a file system watcher for the global MCP settings file
		this.settingsWatcher = this.vscodeAdapter.workspace.createFileSystemWatcher(settingsPattern)

		// Watch for file changes
		const changeDisposable = this.settingsWatcher.onDidChange((uri: { fsPath: string }) => {
		...
		// Watch for file creation
		const createDisposable = this.settingsWatcher.onDidCreate((uri: { fsPath: string }) => {
			if (arePathsEqual(uri.fsPath, settingsPath)) {
				this.debounceConfigChange(settingsPath, "global")
			}
		})

		this.disposables.push(this.vscodeAdapter.createDisposable([changeDisposable, createDisposable, this.settingsWatcher]))
	}
	...
	private async getProjectMcpPath(): Promise<string | null> {
		const workspacePath = this.providerRef.deref()?.cwd ?? this.vscodeAdapter.workspace.workspaceFolders?.[0]?.uri.fsPath ?? process.cwd()
	...
	private createPlaceholderConnection(
		name: string,
		config: z.infer<typeof this.ServerConfigSchema>,
		...
		return {
			...
			server: {
				...
				source,
				projectPath: source === "project" ? this.vscodeAdapter.workspace.workspaceFolders?.[0]?.uri.fsPath : undefined,
				errorHistory: [],
			},
			client: null,
			transport: null,
	...
	private async connectToServer(
		...
		try {
			...
			const configInjected = (await injectVariables(config, {
				env: process.env,
				workspaceFolder: this.vscodeAdapter.workspace.workspaceFolders?.[0]?.uri.fsPath ?? "",
			})) as typeof config
	...
	async updateServerConnections(
		...
		for (const [name, config] of Object.entries(newServers)) {
			...
			let validatedConfig: z.infer<typeof this.ServerConfigSchema>
			try {
	...
	private setupFileWatcher(
		...
		if (config.type === "stdio") {
			...
			if (config.watchPaths && config.watchPaths.length > 0) {
				...
				watchPathsWatcher.on("change", async (changedPath: any) => {
					try {
	...
	async restartConnection(serverName: string, source?: "global" | "project"): Promise<void> {
		...
		if (config) {
			this.vscodeAdapter.window.showInformationMessage(this.t("mcp:info.server_restarting", { serverName }))
			...
			try {
				...
				try {
					...
					this.vscodeAdapter.window.showInformationMessage(this.t("mcp:info.server_connected", { serverName }))
				} catch (validationError) {
	...
	private async notifyWebviewOfServerChanges(): Promise<void> {
		...
		const targetProvider: IProviderAdapter | undefined = this.providerRef.deref()
	...
	public async deleteServer(serverName: string, source?: "global" | "project"): Promise<void> {
		try {
			...
			if (config.mcpServers[serverName]) {
				...
				this.vscodeAdapter.window.showInformationMessage(this.t("mcp:info.server_deleted", { serverName }))
			} else {
				this.vscodeAdapter.window.showWarningMessage(this.t("mcp:info.server_not_found", { serverName }))
			}
		} catch (error) {
	...
	async callTool(
		...
		let timeout: number
		try {
			const parsedConfig = this.ServerConfigSchema.parse(JSON.parse(connection.server.config))
	...
	async handleMcpEnabledChange(enabled: boolean): Promise<void> {
		if (!enabled) {
			...
			if (disconnectionErrors.length > 0) {
				...
				this.vscodeAdapter.window.showWarningMessage(
					this.t("mcp:errors.disconnect_servers_partial", {
			...
			try {
				...
				this.vscodeAdapter.window.showErrorMessage(this.t("mcp:errors.refresh_after_disable"))
			}
		} else {
			...
			try {
				...
				this.vscodeAdapter.window.showErrorMessage(this.t("mcp:errors.refresh_after_enable"))
			}
		}
	}
