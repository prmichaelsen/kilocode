...
this can create inconsistencies when interfacing with other modules that use
...
Note: When interacting with the file system, we still use the native path module
...
// You must import this file in your entry point to have access at runtime
declare global {
	interface String {
		toPosix(): string
	}
...
export function getReadablePath(cwd: string, relPath?: string): string {
	...
	if (arePathsEqual(cwd, path.join(os.homedir(), "Desktop"))) {
		// User opened without a workspace, so cwd is the Desktop. Show the full absolute path to keep the user aware of where files are being created
...
// VSCode-independent version - requires workspace path to be passed in
export const getWorkspacePath = (workspaceFolders?: string[], defaultCwdPath = "") => {
	return workspaceFolders?.at(0) || defaultCwdPath
}

export const getWorkspacePathForContext = (contextPath?: string, workspaceFolders?: string[]): string => {
	// If context path provided, find its workspace
	if (contextPath && workspaceFolders) {
		for (const folder of workspaceFolders) {
			if (contextPath.startsWith(folder)) {
				return folder
			}
		}
	...
	return getWorkspacePath(workspaceFolders)
}