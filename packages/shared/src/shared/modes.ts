...
// Define minimal ExtensionContext interface for web compatibility
...
// Made web-compatible by using generic context interface
export async function getAllModesWithPrompts(context: WebExtensionContext): Promise<ModeConfig[]> {
...
// Made web-compatible by removing addCustomInstructions dependency
export async function getFullModeDetails(
	...
	// Ensure we have a valid base mode
	if (!baseMode) {
		throw new Error(`No mode found for slug: ${modeSlug} and no default mode available`)
	}
	...
	// For web environment, use base custom instructions without file system access
	let fullCustomInstructions = baseCustomInstructions
	if (options?.globalCustomInstructions) {
		fullCustomInstructions = `${baseCustomInstructions}\n\n${options.globalCustomInstructions}`.trim()
	}
